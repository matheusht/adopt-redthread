...
evidence-har-batch:
	@test -n "$(HAR_INPUT_DIR)" || (echo "Usage: make evidence-har-batch HAR_INPUT_DIR=./captures [HAR_BATCH_OUTPUT=runs/har_batches/latest]" && exit 1)
	$(PYTHON) scripts/run_har_evidence_batch.py --input-dir $(HAR_INPUT_DIR) --output-dir $(HAR_BATCH_OUTPUT) --ingestion zapi --redthread-python $(REDTHREAD_PYTHON) --redthread-src ../redthread/src --fail-on-marker-hit
