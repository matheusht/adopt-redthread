...
`make evidence-har-batch-review-workflow` builds sanitized Phase 1-6 follow-up artifacts from the batch output: reviewer packet, boundary plan, approved replay plan, triage automation, readiness gate, and sanitized intent review handoff. The handoff now includes deterministic schema validation, deterministic RedThread execution handoff artifacts, an importable `redthread_intent_evidence.json` package, and a `redthread_importability_report.json` contract harness. Human reviewer summaries and approved non-production execution evidence remain explicit blockers when absent; the workflow does not fabricate validation or execute boundary/live replay.

`make evidence-intent-review HAR_BATCH_OUTPUT=runs/har_batches/latest` builds the Sanitized Intent Review Agent v0 artifacts documented in [`sanitized-intent-review-agent.md`](sanitized-intent-review-agent.md): `intent_review_context.json`, `intent_review.json`, `intent_review.md`, `redthread_evidence_export.json`, `redthread_execution_handoff.json`, `redthread_execution_handoff.md`, `redthread_intent_evidence.json`, `redthread_intent_evidence_validation.json`, and `redthread_importability_report.json`. This is a deterministic sanitized interpretation/export layer only; it does not read raw HARs, execute endpoints, authorize replay, or claim findings. Optional local LLM mode may enrich bounded advisory observations only; it cannot author execution candidates directly.
