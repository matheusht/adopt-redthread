...
- `make evidence-readiness` (one-page sanitized readiness ledger including boundary context intake state; current no-reviewer state remains waiting, not validation)
...
- `make evidence-remediation-queue` (ordered local work queue from sanitized readiness/distribution blockers, including boundary context intake blockers; does not execute probes or contact reviewers)
