...
def build_evidence_report(run_dir: str | Path, output_path: str | Path | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "- Evidence envelope: this report describes only the tested input, replay path, RedThread evidence, and local gate result for this run; it is not a whole-app safety proof.",
        "- RedThread role: RedThread replay/dry-run is evidence consumed by this report, not the final bridge gate owner yet.",
        "- Local bridge role: the local Adopt RedThread bridge currently emits the final `approve` / `review` / `block` verdict.",
        "- `approve`: ship candidate for this tested evidence envelope only.",
        "- `review`: change/review checkpoint before ship; expected for write-capable or human-sensitive paths.",
        "- `block`: do not ship from this run until required context, replay, or evidence blockers are resolved.",
        "- Finding wording: auth/replay/context failures and insufficient evidence are not reported as confirmed vulnerabilities unless `confirmed_security_finding` is true.",
        "",
        "## Decision",
        "",
