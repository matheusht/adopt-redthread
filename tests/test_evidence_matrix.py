...
class EvidenceMatrixTests(unittest.TestCase):
    def test_matrix_assigns_approve_review_block_agents(self) -> None:
        ...
        self.assertIn("Reviewer action", matrix_md)
        self.assertIn("ship candidate: local gate approved", matrix_md)
        self.assertIn("change/review before ship: manual_review_required_for_write_paths", matrix_md)
        self.assertIn("block until approved context/replay gap is resolved: missing_write_context", matrix_md)
        self.assertIn("RedThread replay/dry-run is evidence", matrix_md)
        self.assertIn("## How to read this matrix", matrix_md)
        self.assertIn("Each row is a tested evidence envelope", matrix_md)
        self.assertIn("Finding type separates confirmed findings", matrix_md)
