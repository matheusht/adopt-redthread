...
class EvidenceReportTests(unittest.TestCase):
    def test_block_report_names_exact_workflow_blocker_plainly(self) -> None:
        ...
        self.assertIn("## Silent reviewer checklist", report)
        self.assertIn("Ship, change, or block? block until approved context/replay gap is resolved: missing_write_context", report)
        self.assertIn("What evidence should I trust most? live workflow replay status successful:0,blocked:1,aborted:0; response bindings applied:1/1; RedThread replay passed:True; RedThread dry-run rubric:n/a", report)
        self.assertIn("What is still unclear or weak? auth_or_replay_blocked,tenant_user_boundary_unproven,workflow_blocked", report)
        self.assertIn("What evidence should I collect next? supply approved non-production staging write context and rerun workflow replay", report)
        self.assertIn("What changes force a rerun? rerun when tool/action schemas, operation order, action class, or exposed scope changes", report)
        self.assertIn("run the targeted ownership-boundary probe: Verify user/tenant identifiers", report)
        self.assertIn("Confirmed issue, auth/replay failure, or insufficient evidence? auth/replay/context failure:missing_write_context; not a confirmed vulnerability", report)
        self.assertIn("Repeat before release? Rerun this evidence path when tool scopes, auth/write context, binding behavior, or boundary selectors change before release.", report)
        self.assertIn("## How to read this evidence", report)
        self.assertIn("not a whole-app safety proof", report)
        self.assertIn("confirmed vulnerabilities unless `confirmed_security_finding` is true", report)
        ...
        self.assertIn("## Next evidence to collect", report)
        self.assertIn("supply approved non-production staging write context and rerun workflow replay", report)
        self.assertIn("resolve the emitted workflow blocker category and rerun live workflow replay", report)
        self.assertIn("run the targeted ownership-boundary probe: Verify user/tenant identifiers", report)
        self.assertIn("## Rerun triggers", report)
        self.assertIn("rerun when approved auth, session, target environment, or staging write context changes", report)
        self.assertIn("rerun when user, tenant, resource, route, or ownership-boundary selectors change", report)
