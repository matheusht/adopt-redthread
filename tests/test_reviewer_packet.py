...
class ReviewerPacketTests(unittest.TestCase):
    def test_packet_points_to_sanitized_artifacts_and_questions(self) -> None:
        ...
        self.assertIn("## Session-local path note", packet_md)
        self.assertIn("Expected cold-review conclusion", packet_md)
        self.assertIn("Fill `../filled_reviewer_observation.md`; it starts blank", packet_md)
        ...
        self.assertIn("Give one isolated review_N session folder to the reviewer", packet_md)
        ...
        self.assertIn("This file starts as a blank template", observation_md)
